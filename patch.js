    ...
    if (isBoss) {
      ...
      if (currentSubStage === 9) {
        ...
        if (currentIdx >= 0 && currentIdx < eraOrder.length - 1) {
          const nextEra = eraOrder[currentIdx + 1] as import("./types").EraId;
        ...
        // Reset distance for next era loop or just bump it slightly so it doesn't trigger again immediately?
        // Actually, let's reset distance to 0 when they change era manually? Or maybe they auto-transition?
        // "When they beat the era, they should be able to win enough coins... to open up the next era."
        // We'll bump distance so we go to 1000.1, meaning stage 10. They can keep running.
